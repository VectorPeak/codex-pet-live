...
#from CodexPetLive.extra_windows import SpriteDialogue
from CodexPetLive.custom_widgets import SpriteDialogue, MenuSlider
from CodexPetLive.utils import SubPet_Manager
...
class SpriteAccessory(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        宠物组件
        """
        super(SpriteAccessory, self).__init__(parent) #, flags=Qt.WindowFlags())
    ...
    def setup_accessory(self, acc_act, pos_x, pos_y):
        ...
        if acc_act.get('name','') == 'item_drop':
            ...
            self.acc_dict[acc_index] = SpriteDialogue(acc_index, acc_act['msg_dict'],
                                                  pos_x, pos_y)
        ...
        ''' mouse decorator not implemented
            ...
            for qacc in self.acc_dict:
                if not isinstance(self.acc_dict[qacc], SpriteMouseDecor):
                    continue
            ...
            # 激活挂件
            self.acc_dict[acc_index] = SpriteMouseDecor(acc_index, acc_act['config'])
            self.acc_dict[acc_index].acc_withdrawed.connect(self.acc_withdrawed)
        '''
    ...
    def animation(self):
        ...
        if self.interact is None:
            if self.dist_listen:
                ...
                if self.act_name != 'sprite_follow_x':
                    self.empty_interact()
                self.act_name = 'sprite_follow_x'
                self.default_act("sprite_follow_x")
...
class SpriteMouseDecor(QWidget):
    closed_acc = Signal(str, name='closed_acc')
    acc_withdrawed = Signal(str, name='acc_withdrawed')
    ...
    def __init__(self, acc_index,
                 config,
                 parent=None):
        super(SpriteMouseDecor, self).__init__(parent)
