...
from CodexPetLive.custom_widgets import SystemTray
from .custom_roundmenu import RoundMenu

from CodexPetLive.conf import *
from CodexPetLive.utils import *
from CodexPetLive.modules import *
from CodexPetLive.Accessory import MouseMoveManager
from CodexPetLive.custom_widgets import RoundBarBase, LevelBadge
from CodexPetLive.bubbleManager import BubbleManager
from CodexPetLive.runtime_paths import runtime_data_dir
...
# version
codexpetlive_version = settings.VERSION
vf = open(os.path.join(runtime_data_dir(configdir), 'version'), 'w')
vf.write(codexpetlive_version)
vf.close()
...
class PetWidget(QWidget):
    ...
    def _set_Statusmenu(self):
        ...
        #h_box1.addStretch(1)
        self.pet_hp = SpriteHpBar(self, minimum=0, maximum=100, objectName='PetHP')
        ...
        self.pet_fv = SpriteFvBar(self, minimum=0, maximum=100, objectName='PetEM')
    ...
    def _change_status(self, status, change_value, from_mod='Scheduler', send_note=False):
        ...
        if status == 'hp' and from_mod == 'Scheduler': # avoid being called in both hp and fv
            # Random Bubble
            scheduled_probability = settings.PP_BUBBLE
            if settings.llm_provider_config.get('enabled', False):
                scheduled_probability = settings.llm_provider_config.get(
                    'scheduled_bubble_probability',
                    settings.PP_BUBBLE,
                )
            if random.uniform(0, 1) < scheduled_probability:
    ...
    def patpat(self):
        ...
        if settings.llm_provider_config.get('enabled', False):
            pat_bubble_probability = settings.llm_provider_config.get('pat_bubble_probability', 0.6)
        else:
            pat_bubble_probability = max(0.0, (1 - sys_pp_audio) * 0.5)

        if random.uniform(0, 1) < pat_bubble_probability:
            self.bubble_manager.trigger_patpat_random()
        elif not settings.llm_provider_config.get('enabled', False) and random.uniform(0, 1) > sys_pp_audio:
