...
from qfluentwidgets import (RoundMenu, FluentIcon, Action, AvatarWidget, BodyLabel, ToolButton,
                            ...
                            ExpandGroupSettingCard, RadioButton, ColorDialog, PasswordLineEdit)
...
from CodexPetLive.conf import load_ItemMod
from CodexPetLive.utils import text_wrap

from sys import platform
basedir = settings.BASEDIR
module_path = os.path.join(basedir, 'CodexPetLive/SpriteSettings/')
...
def _first_valid_role_frame(parent_dir, role_folder):
    role_dir = os.path.join(basedir, f"res/{parent_dir}", role_folder)
    action_dir = os.path.join(role_dir, "action")
    if not os.path.isdir(action_dir):
        return None

    act_conf = _load_json_safely(os.path.join(role_dir, "act_conf.json"))
    pet_conf = _load_json_safely(os.path.join(role_dir, "pet_conf.json"))
    prefixes = []

    default_act = pet_conf.get("default")
    for act_name in (default_act, "default", "stand", "idle", "onfloor", "drag"):
        act_config = act_conf.get(act_name)
        if isinstance(act_config, dict):
            prefixes.append(act_config.get("images"))
        elif isinstance(act_name, str):
            prefixes.append(act_name)

    checked = set()
    candidates = []
    for prefix in prefixes:
        if not prefix or prefix in checked:
            continue
        checked.add(prefix)
        candidates.append(os.path.join(action_dir, f"{prefix}_0.png"))
        candidates.extend(sorted(glob.glob(os.path.join(action_dir, f"{prefix}_*.png"))))

    candidates.extend(sorted(glob.glob(os.path.join(action_dir, "*.png"))))
    for path in candidates:
        image = _load_image_safely(path)
        if not image.isNull():
            return path
    return None


def _visible_avatar_placeholder(edge_size=65):
    image = QImage(edge_size, edge_size, QImage.Format_ARGB32)
    image.fill(Qt.transparent)

    painter = QPainter(image)
    painter.setRenderHint(QPainter.Antialiasing)
    painter.setBrush(QBrush(QColor("#3b4754")))
    painter.setPen(Qt.NoPen)
    painter.drawEllipse(0, 0, edge_size, edge_size)

    font = QFont()
    font.setBold(True)
    font.setPointSize(max(12, edge_size // 3))
...
class SpriteRangeSettingCard(SettingCard):
...
class SpriteComboBoxSettingCard(SettingCard):
...
#===========================================================
#    Customized line-edit setting card
#===========================================================

class SpriteLineEditSettingCard(SettingCard):
    """ Setting card with a single-line text input """

    textEdited = Signal(str, name="textEdited")
    editingFinished = Signal(str, name="editingFinished")

    def __init__(self, icon: Union[str, QIcon, FluentIconBase], title, content=None,
                 text='', placeholder='', password=False, parent=None):
        super().__init__(icon, title, content, parent)
        self.lineEdit = PasswordLineEdit(self) if password else LineEdit(self)
        self.lineEdit.setMinimumWidth(260)
        self.lineEdit.setText(text or '')
        self.lineEdit.setPlaceholderText(placeholder)

        if password and hasattr(self.lineEdit, "setEchoMode"):
            self.lineEdit.setEchoMode(QLineEdit.Password)

        if hasattr(self.lineEdit, "setClearButtonEnabled"):
            self.lineEdit.setClearButtonEnabled(True)

        self.hBoxLayout.addWidget(self.lineEdit, 0, Qt.AlignRight)
        self.hBoxLayout.addSpacing(16)

        self.lineEdit.textChanged.connect(self.textEdited)
        self.lineEdit.editingFinished.connect(self.__onEditingFinished)

    def __onEditingFinished(self):
        self.editingFinished.emit(self.lineEdit.text())
...
#===========================================================
#    Customized push-button setting card
#===========================================================

class SpritePushButtonSettingCard(SettingCard):
    """ Setting card with a right-aligned push button """

    clicked = Signal(name="clicked")

    def __init__(self, button_text, icon: Union[str, QIcon, FluentIconBase],
                 title, content=None, primary=False, parent=None):
        super().__init__(icon, title, content, parent)
...
class SpriteToolButtonCard(SettingCard):
...
class SaveCardGroup(QWidget):
    ...
    def __init__(self, title: str, sizeHintSprite, parent=None):
        super().__init__(parent=parent)
        self.sizeHintSprite = sizeHintSprite
        self.titleLabel = QLabel(title, self)
        self.titleLabel.setObjectName("cardGroupTitleLabel")
        self.vBoxLayout = QVBoxLayout(self)
    ...
    def adjustSize(self):
        width = self.sizeHintSprite[0] - 50
...
class CharLine(SimpleCardWidget):
    ...
    def __init_InfoList(self):
        ...
        if not os.path.exists(infoFile):
            ...
            infoConfig = _load_json_safely(infoFile)
            self.chrName = infoConfig.get("petName", self.chrFolder)
            configuredPfpPath = infoConfig.get("pfp", None)
            if configuredPfpPath:
                candidatePfpPath = os.path.join(basedir, f'res/{self.parentDir}', self.chrFolder, 'info', configuredPfpPath)
                if os.path.exists(candidatePfpPath):
                    pfpPath = candidatePfpPath

        if pfpPath is None:
            pfpPath = _first_valid_role_frame(self.parentDir, self.chrFolder)

        # Character pfp
        image = _load_image_safely(pfpPath)
        if image.isNull():
            image = _visible_avatar_placeholder()
...
def AvatarImage(image, edge_size=65, frameColor=QColor(255, 255, 255, 0)): #"#ffffff"):
    ...
    if image.isNull():
        image = _visible_avatar_placeholder(edge_size)

    image = image.convertToFormat(QImage.Format_ARGB32)
    circle_r = max(1, min(image.width(), image.height()))
    crop_x = max(0, (image.width() - circle_r) // 2)
    crop_y = max(0, (image.height() - circle_r) // 2)
    image = image.copy(crop_x, crop_y, circle_r, circle_r)

    avatar = QImage(circle_r, circle_r, QImage.Format_ARGB32)
    ...
    painter.setRenderHint(QPainter.Antialiasing)
    path = QPainterPath()
    path.addEllipse(QRectF(margin, margin, circle_r-2*margin, circle_r-2*margin))
    painter.setClipPath(path)
    painter.drawImage(0, 0, image)
    painter.end()

    label.setPixmap(QPixmap.fromImage(avatar))
