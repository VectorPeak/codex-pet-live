...
from CodexPetLive.runtime_paths import app_config_dir, runtime_data_dir
from CodexPetLive.utils import text_wrap, get_child_folder
...
class PetConfig:
    ...
    def init_config(cls, pet_name: str, pic_dict: dict):
        ...
        with open(path, 'r', encoding='utf-8-sig') as f:
            ...
            act_conf = dict(_read_json_file(act_path))
            act_dict = {}
            ...
            if os.path.isfile(msg_file):
                msg_data = dict(_read_json_file(msg_file))
...
def CheckCharFiles(folder):
    ...
    try:
        ...
        pet_conf = _read_json_file(path)
    except:
        return 1, None

    try:
        path = os.path.join(folder, 'act_conf.json')
        act_conf = _read_json_file(path)
    except:
        return 2, None
...
class ActData:
    ...
    def __init__(self, petsList):
        ...
        self.file_path = os.path.join(runtime_data_dir(configdir), 'act_data.json')
    ...
    def init_config(self):
        if os.path.isfile(self.file_path):
            ...
            try:
                allAct_params = _read_json_file(self.file_path)
                self.fileGood = True
            except:
                #File broken
                allAct_params = {}
    ...
    def _check_actlist(self, petname, act_params, fv_lvl):
        ...
        pet_conf = _read_json_file(pet_conf_file)
        all_names_in_conf = []
...
class PetData:
    ...
    def __init__(self, petsList):
        ...
        self.file_path = os.path.join(runtime_data_dir(configdir), 'pet_data.json') #%(self.petname)
        self.petsList = petsList
        self.current_pet = petsList[0]
    ...
    def init_data(self):
        ...
        if os.path.isfile(self.file_path):
            # Check file integrity
            try:
                allData_params = _read_json_file(self.file_path)
                self.saveGood = True
            except:
...
class TaskData:
    ...
    def __init__(self):
        ...
        self.file_path = os.path.join(runtime_data_dir(configdir), 'task_data.json')
        self.init_data()
        self.save_data()
    ...
    def init_data(self):
        ...
        if os.path.isfile(self.file_path):
            # Check file integrity
            try:
                self.taskData = _read_json_file(self.file_path)
                self.stateGood = True
            except:
                #File broken (seen by a few users)
    ...
    def init_data(self):
        ...
        for i, itemFolder in enumerate(sorted_itemMods):
            ...
            if os.path.exists(info_file):
                info = dict(_read_json_file(info_file))
            ...
            item_conf = dict(_read_json_file(conf_file))
...
def load_ItemMod(configPath, HUNGERSTR='Satiety', FAVORSTR='Favorability'):
    ...
    item_conf = dict(_read_json_file(configPath))
...
def checkItemMOD(itemFolder):
    ...
    try:
        item_dict = dict(_read_json_file(configFile))
    except:
        return 1, None
