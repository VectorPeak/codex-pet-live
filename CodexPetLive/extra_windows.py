...
data_dir = runtime_data_dir(configdir)
...
class Remindme(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        if os.path.isfile(os.path.join(data_dir,'remindme.txt')):
            f = open(os.path.join(data_dir,'remindme.txt'),'r', encoding='UTF-8')
            texts = f.read()
            f.close()
            texts = texts.lstrip('\n')
            self.e2.setPlainText(texts)
        else:
            f = open(os.path.join(data_dir,'remindme.txt'),'w', encoding='UTF-8')
            f.write('')
            f.close()
...
class SpriteDialogue(QWidget):
    ...
    def __init__(self, acc_index,
                 ...
                 timeout=5000,
                 parent=None):
        super(SpriteDialogue, self).__init__(parent)
