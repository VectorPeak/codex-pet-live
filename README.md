...
普通用户只需要下载 Windows 发布包，解压后双击 `PeakDeskSprite.exe`，桌宠就会出现在桌面上。你可以在设置面板里切换角色、调整气泡行为、配置 LLM 服务，或者把它当作一个日常陪伴型桌面应用使用

如果你已经有 Codex 生成的 hatch-pet 包，可以通过内置转换工具把 `pet.json` 和 `spritesheet.webp` 转成 CodexPetLive 可识别的角色模块，然后在角色管理里启用它。完整转换流程见 [HatchPet 转换说明](docs/hatchpet_converter.md)
