...
PeakDeskSprite 是一个基于 PySide6 的桌面宠物开发框架，致力于为开发者提供创造桌面宠物的底层软件。它把角色素材、动作状态、交互事件、背包物品、系统通知和可选 LLM 聊天能力组织在同一个桌面应用里，让一个桌宠既能被直接使用，也能继续扩展成新的角色、动作和玩法

它的定位不是一个只绑定某个形象的小程序，而是一个支持高度 DIY 的桌宠底座。使用者可以把它当作桌面陪伴应用，素材作者可以把它当作角色素材实验室，开发者则可以把它当作交互 Demo 原型或轻量桌面工具的起点
...
| 大模型服务与气泡 | 聊天面板 |
| --- | --- |
| ![大模型服务配置](docs/preview_img/llm-provider.png) | ![LLM 聊天面板](docs/preview_img/llm-chat.png) |
| 配置 OpenAI-compatible 服务商、接口地址、模型名、API Key 和环境变量读取策略 | 角色聊天界面支持上下文消息、角色设定入口、聊天记录保存和 AI 内容提示 |

| 气泡行为 | 角色管理 |
| --- | --- |
| ![气泡行为设置](docs/preview_img/llm-bubble-settings.png) | ![角色管理](docs/preview_img/role-manager.png) |
| 调整气泡模式、主动聊天、冷却时间、摸摸气泡概率和状态气泡概率 | 添加、切换和启动不同角色，角色素材可以通过资源目录或导入流程扩展 |
...
当前公开仓库保留 `v0.8.5` 版本 tag，但 GitHub Releases 页面可能暂时没有可下载附件。如果 [Releases](https://github.com/VectorPeak/PeakDeskSprite/releases) 中提供了 Windows 发布包，文件名通常类似 `PeakDeskSprite-vX.Y.Z-windows-x64.zip`，解压后双击 `PeakDeskSprite.exe` 即可启动桌宠

如果没有可下载的 release 附件，请按下面的“从源码运行”方式启动项目。开发者如果需要自己构建 Windows zip 包，请参考 [Windows 发布清单](docs/release_checklist.md)
...
<table>
  ...
  <tbody>
    ...
    <tr><td width="180" nowrap>打包与工具</td><td>PyInstaller 打包</td><td>发布流程围绕 Windows 可执行文件和发布包审计展开</td></tr>
